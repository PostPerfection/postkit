...
### Fixed
- **A slope hint 511 above a frame's own threshold no longer exceeds the byte cap**: grok narrows its rate search to 512 either side of the hint the caller passes, and that one hint made it disable rate control and write every coding pass, so the frame came out over the cap and the encode failed at the writer's size check. The search fix is in grok. postkit now encodes the same frame at every offset around the bracket edges through the FFI and checks the byte count does not move.
- **The grok player no longer drops frames when the OS wakes it late**: the scheduler slept until the next frame was due, and on the macOS CI runner a timed wait oversleeps by several times its length, so the scheduler woke after the frame's slot and counted a drop. While playing it now waits at most 4 ms at a time. Idle and paused waits are unchanged.
- **A grok preview decode scale change no longer drops frames**: switching between full, half and quarter emptied the decoded frame cache the way a seek does, so the picture had nothing to show until the first frames came back at the new scale, which the sound clock counts as dropped frames and the old wall clock showed as a freeze. The change now keeps every decoded frame, bumps the job generation so the queued jobs are resubmitted at the new scale, and lets results from jobs that started before the change still land. The window is re-requested at the new scale and each frame is replaced as its new result arrives, so the picture runs on the old scale for a moment instead of stopping. A change while paused holds the frame on screen until the current one is decoded at the scale that was asked for.
- **Grok preview sound no longer falls behind the picture**: the picture stepped on a wall clock while the sound queue drained at the device rate, and silence written on an empty queue never moved the sound's own position, so every starved callback pushed the sound later for good. The picture now follows the sound device's clock whenever a stream is live, a starved queue makes the feeder skip to the frame the device will reach when the queue drains instead of lagging, the queue holds half a second and fills from load rather than from the first play, and the stream asks the device for the PCM's own sample rate and linearly interpolates to the device rate only when no configuration supports it.
