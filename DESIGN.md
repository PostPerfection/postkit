...
Encoding:
- encode: J2K encode orchestration (detection, parallel encode, progress). `StreamEncodeOptions.source_colour` is `DisplayRgb`, `DciLut(path)` or `AlreadyPq` plus an optional `codestream_byte_cap`: grok gets `--xyz` if and only if the variant applies the XYZ transform and the LUT runs in the same ffmpeg pass, so XYZ-transformed frames can never be labelled PQ. The in-process and subprocess encoders refuse the HDR options loudly
